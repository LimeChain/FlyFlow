...
9. `npm run compile` succeeds with zero warnings; `artifacts/ETHDILITHIUM/src/ZKNOX_keccak_prng.sol/*.json` and `.../ZKNOX_ethdilithium.sol/*.json` exist. [AC-1-9]
...
truths:
  ...
  - "`npx hardhat compile` completes with zero warnings and generates artifacts for both `ETHDILITHIUM/src/ZKNOX_keccak_prng.sol` and `ETHDILITHIUM/src/ZKNOX_ethdilithium.sol` under Solidity 0.8.34."
