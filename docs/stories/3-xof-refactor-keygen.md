...
- [ ] **Task 2: `mldsa-encoding.ts` XOF-factory refactor + SHAKE/Keccak adapters + post-refactor NIST regression assertion**
  ...
  - Why: The load-bearing refactor. Concretely:
    ...
    4. **Migrate the single existing caller** at `test/fixtures/mldsa.ts:49`: `const encoded = preparePublicKeyForDeployment(rawPublicKey, shake128XofFactory, shake256XofFactory);`. After this line is migrated, run `npm test` against the existing NIST `MlDsaAccount` test suite — AC-D-1 requires byte-identical pass.
    ...
    6. **Write `mldsa-encoding.nist-regression.test.ts`** that reads `test/fixtures/kat/nist-regression/vectors.json` (the Task 1 artifact) and asserts `preparePublicKeyForDeployment(pk, shake128XofFactory, shake256XofFactory) === expectedReshapedPk` byte-for-byte for all 100 vectors. On mismatch, HALT per refactor-rollback protocol.
...
2. **Post-refactor NIST regression passes.** `npx hardhat test test/signers/mldsa-encoding.nist-regression.test.ts` — all 100 vectors byte-identical between `preparePublicKeyForDeployment(pk, shake128XofFactory, shake256XofFactory)` output and the frozen fixture's `expectedReshapedPk`. [AC-3-3 half 2]
...
truths:
  - "`preparePublicKeyForDeployment(pk, shake128XofFactory, shake256XofFactory)` post-refactor produces byte-identical output to the frozen NIST regression fixture (`test/fixtures/kat/nist-regression/vectors.json`) for all 100 `PQCsignKAT_Dilithium2.rsp` pks — AC-3-3."
