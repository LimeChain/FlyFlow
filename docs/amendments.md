...
## A-005: mldsa-eth fixture `rnd` field must come from TWO separate `random_bytes(32)` calls, not bytes[32:64] of a single 64-byte call

- **Story:** 4 (Signer port + G2 KAT) — caught at Task 1 kickoff before any Task-4 G2 KAT test was written.
- **Origin:** Story 1 (Fixture-gen CLI) — `scripts/generate-kat-fixtures.ts:process_vector` used `bs = drbg.random_bytes(64); rnd = bs[32:64]`.
- **Date:** 2026-04-18
- **Classification:** Rule 3 (Significant — fixture contract change; affects `test/fixtures/kat/mldsa-eth/vectors.json` byte contents for the `rnd` column only, keeping zeta/pk/sk/reshapedPk/message/signature untouched).
- **Affects:** Story 1's §"Verified Interfaces" `AES256_CTR_DRBG.random_bytes` entry (line 70 of `docs/stories/1-fixture-gen-cli.md`); architecture §"Data Models" §"KAT fixture JSON (DD-7 LOCKED)"; the fixture's `source.drbgDerivation` metadata field. Unblocks Story 4 AC-4-1 (G2 byte-identity).

### Original (Story 1 claim + fixture field)

Story 1's §"Verified Interfaces" at line 70:
...
And the fixture's metadata:

```json
"drbgDerivation": "AES256_CTR_DRBG(drbgSeed).random_bytes(64) → ζ=[0:32], rnd=[32:64]"
```

Implementation (`scripts/generate-kat-fixtures.ts:648-650`):
...
### Actual (NIST SP 800-90A § 10.2.1.5.1 `CTR_DRBG_Generate_algorithm`)

`AES256_CTR_DRBG.random_bytes` runs `__ctr_drbg_update(additional)` at the END of every call (`ETHDILITHIUM/pythonref/dilithium_py/drbg/aes256_ctr_drbg.py:132`), which re-derives the internal `(key, V)` state via 3 AES-ECB blocks ⊕ `provided_data`. Consequently:

- `drbg.random_bytes(64)` produces **4 AES blocks** then updates once.
- `drbg.random_bytes(32); drbg.random_bytes(32)` produces **2 blocks + update + 2 blocks + update**.

The second-half 32 bytes DIFFER between these paths. Empirically (seed `0615…FFA1`, `.rsp` vec 0):

| Path | bytes[0:32] (zeta) | bytes[32:64] |
|------|-------------------|--------------|
| single `random_bytes(64)` | `7c9935a0…36737f2d` | `b505d7cf…4f29202a` |
| two `random_bytes(32)` calls | `7c9935a0…36737f2d` | **`8626ed79…872bfb8f`** |

`zeta` is identical (first 2 blocks); `rnd` diverges after the intermediate `__ctr_drbg_update`.

Python's `Dilithium.sign(sk, msg, ...)` (non-deterministic branch, `dilithium.py:442`) uses `rnd = self.random_bytes(32)` as a fresh call AFTER keygen's `self.random_bytes(32)`. Therefore the ETH KAT signatures in `PQCsignKAT_Dilithium2_ETH.rsp` (generated by `test_dilithium.py::test_dilithium2_eth` — two separate sign/keygen DRBG consumes under one seeded instance) were produced against the **two-call** `rnd`, not the `bytes[32:64]` rnd Story 1 stored.

### Evidence

Empirical cross-check against `ETHDILITHIUM/pythonref/dilithium_py/drbg/aes256_ctr_drbg.py` executed in the pinned submodule, using the published vec-0 seed:

```
single_64 bytes[0:32]  = 7c9935a0b07694aa0c6d10e4db6b1add2fd81a25ccb148032dcd739936737f2d
single_64 bytes[32:64] = b505d7cfad1b497499323c8686325e4792f267aafa3f87ca60d01cb54f29202a  ← WRONG rnd
two_32 first 32        = 7c9935a0b07694aa0c6d10e4db6b1add2fd81a25ccb148032dcd739936737f2d
two_32 second 32       = 8626ed79d451140800e03b59b956f8210e556067407d13dc90fa9e8b872bfb8f  ← CORRECT rnd
```

Empirical G2 byte-identity check: with the corrected rnd, `signWithXofInstrumented(sk, msg, rnd_corrected, new Uint8Array(0), keccakXofFactory)` produces byte-identical output to vec-0's `.signature` field across the first 3 vectors tested at Task-1 kickoff (`scripts/_smoke-g2.ts` — 1/9/4 rejection iterations, all byte-identical). Task 4 will formalize this across all 100 vectors.

### Impact

- **Story 1** — `scripts/generate-kat-fixtures.ts:process_vector` fix: replace `bs = drbg.random_bytes(64); zeta = bs[0:32]; rnd = bs[32:64]` with two separate `drbg.random_bytes(32)` calls. Fixture metadata string updated accordingly.
- **`test/fixtures/kat/mldsa-eth/vectors.json`** — regenerated; the 100 `rnd` fields and the `source.drbgDerivation` description change, all other per-vector fields (zeta / publicKey / secretKey / reshapedPublicKey / message / signature) byte-identical to pre-A-005 state (verified via `git diff --stat`: 101 insertions / 101 deletions, scoped entirely to the `rnd` column + schema description).
- **Story 1 AC-1-2 (determinism)** — UNAFFECTED. Re-running the CLI on the pinned submodule produces byte-identical output every time; the change is a one-time semantic correction, not a source of nondeterminism.
- **Story 1 AC-1-1 (fixture shape)** — UNAFFECTED. The field list `(id, drbgSeed, zeta, rnd, publicKey, secretKey, reshapedPublicKey, message, signature)` is unchanged; only the **value** of the `rnd` field changes, not its presence or type.
- **Story 3 G1 KAT (AC-3-1)** — UNAFFECTED. G1 consumes `zeta` (unchanged) to reproduce `(publicKey, secretKey)` (unchanged). Re-running `test/signers/ml-dsa-eth.keygen.kat.test.ts` post-regen continues to pass 100/100 byte-identical (verified at Task 1 kickoff — full test suite 76/76 post-regen).
- **Story 4 AC-4-1 (G2 KAT)** — UNBLOCKED by this amendment. Without A-005 the `rnd` field is semantically incompatible with the `.rsp`-sourced signature field, making G2 byte-identity unachievable.
- **Architecture §"Data Models" §DD-7 / §"KAT fixture JSON"** — the `drbgDerivation` description line needs an update if it explicitly spells the single-call shape; the architecture text lives downstream of the fixture and tracks the amended shape going forward.

### Rationale

Story 1 drafted the rnd derivation by reading `dilithium.py:442` and mentally modelling the DRBG as stateless-within-a-seed (`random_bytes(64)` ≡ `random_bytes(32) || random_bytes(32)`). That mental model is incorrect for NIST SP 800-90A CTR-DRBG: `CTR_DRBG_Generate_algorithm` (§10.2.1.5.1) performs an unconditional `CTR_DRBG_Update` at the end of every generate call, which re-derives `(key, V)` and therefore advances the output stream in a non-monotonic way between separate calls.

The bug hid through Story 1-3 because none of the completed tests consume the `rnd` field. G1 (Story 3) uses `zeta` only; NIST regression uses the `.rsp` NIST-variant `pk` only; xof-isolation uses `publicKey` from the mldsa-eth fixture only. Task 1 of Story 4 is the first time `rnd` feeds a consumer; the smoke check surfaced the mismatch before any G2 test landed.

Caught, verified, and corrected before any downstream consumer was wired to the wrong values.

### Resolution

- `scripts/generate-kat-fixtures.ts` updated: `process_vector()` now runs `zeta = drbg.random_bytes(32); rnd = drbg.random_bytes(32)` (two separate calls) and the `source.drbgDerivation` metadata string documents the A-005 semantics.
- `test/fixtures/kat/mldsa-eth/vectors.json` regenerated in place; diff scoped to the 100 `rnd` fields + schema description.
- Full test suite re-run post-regeneration: 76/76 passing (no regression).
