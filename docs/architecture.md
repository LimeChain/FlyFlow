...
| JS signer (production surface) | Hedged keygen + sign over a userOp hash under Falcon-ETH. Exposes `keygen(): Keypair` and `signUserOp(sk, userOp, entryPoint, chainId)` with fresh entropy via `crypto.getRandomValues`. | `test/signers/falcon-eth.ts` (new) | `falcon-eth.core.ts`, `keccak-prg.ts`, `mldsa-encoding.ts` (shared `keccakXofFactory`) [**A-006 SUPERSEDES**: post-Strategy-E deps are `falcon-eth.core.ts` (`falcon512paddedEth`) + `falcon-encoding.ts` (`encodeSignatureForZKNOX`); no `keccakXofFactory` at production — see `docs/amendments.md` §A-006.] |
...
| Falcon-ETH shared core | Forked math from `@noble/post-quantum/src/falcon.ts` — reuses `ffSampling`, `splitFFT`, `mergeFFT`, NTT, `Float`, `BNORM_MAX`, `SIGMA_MIN`, `INV_SIGMA`, `COMPLEX_ROOTS` verbatim. Forks `HashToPoint` and sampler classes to accept an `XofFactory` (DD-10). Exposes `signWithXof(sk, msg, salt, xofFactory)` + `signWithXofInstrumented(...)` (sibling export per C-8, no module-level XOF state). Module header carries `@delta-from-falcon` JSDoc enumerating byte-level differences from NIST (AC-D-3). [**A-006 SUPERSEDES**: Strategy E — no TS-side signer source-transplant; fork-side HashToPoint injection at `github.com/LimeChain/noble-post-quantum-eth`. Our `falcon-eth.core.ts` exports `hashToPointEVM` (Story 2-2) + `falcon512paddedEth = genFalcon({ ...falcon512paddedOpts, hashToPoint: hashToPointEVM })`. `signWithXof` / `signWithXofInstrumented` / forked samplers / `XofFactory` parameterization / `@delta-from-falcon` JSDoc are all DROPPED. See `docs/amendments.md` §A-006.] | `test/signers/falcon-eth.core.ts` (new) | `@noble/post-quantum/src/falcon.ts` (source copy of math-only subset), `@noble/curves/abstract/fft.js`, `keccak-prg.ts` |
| Keccak-CTR-PRNG primitive | **Reused as-is from mldsa-eth.** Stateful `inject`/`flip`/`extract` construction. Lifecycle guards (inject-after-flip throws `PrgLifecycleError`). Byte-compatibility with ETHFALCON's `Keccak256PRNG(a, b)` [**A-004**: actual ETHFALCON class is `KeccakPRNG()` (0-arg); `Keccak256PRNG(a, b)` is the ETHDILITHIUM wrapper — see `docs/amendments.md` A-004] must be verified by **G1** per DD-13 before trusting for G3 (keygen) / G4 (signer) / G5 (pk-transform). | `test/signers/keccak-prg.ts` (existing, unchanged) | viem `keccak256` |
...
| XOF factory (Keccak variant) | **Reused as-is from mldsa-eth.** `keccakXofFactory: (seed) => XofReader` with `id: "keccak-prg"` discriminant. Called at every falcon-eth call-site; never stored in module-level state. [**A-006 SUPERSEDES**: not called at falcon-eth signer call-sites post-Strategy-E. The fork's HashToPoint injection consumes our repo's `hashToPointEVM` (Keccak256-based but NOT XofFactory-shaped). `keccakXofFactory` remains used by `mldsa-encoding.ts` and indirectly by `hashToPointEVM`'s internal `keccak256(salt||msg)` one-shot absorb — but not as a `XofFactory`-typed parameter. See `docs/amendments.md` §A-006.] | `test/signers/mldsa-encoding.ts:keccakXofFactory` (existing, unchanged) | `keccak-prg.ts` |
...
- `XofFactory` is passed as a **function parameter at every call-site** — no module-level mutable state. AC-A-5 extends the existing xof-lifecycle grep glob (`^(let|var) _?xof`) to `test/signers/{mldsa,falcon}-eth*.ts`. [**A-006 SUPERSEDES**: falcon-eth has no `XofFactory` call-site post-Strategy-E (the fork's factory-level `hashToPoint?` injection handles the variant-specific HashToPoint; no runtime XOF swap). AC-A-5 grep stays relevant for ml-dsa-eth's path. See `docs/amendments.md` §A-006.]
...
```jsonc
{
  ...
  "source": {
    ...
    "drbgDerivation": "AES256_CTR_DRBG(drbgSeed).random_bytes(N) — N TBD by Python-ref audit (PRE_G4_DRBG_PROBE)",  // [A-005]: Python runs DRBG at fixture-gen for keygen+sign OUTPUT capture. Separately, TS tests replay DRBG via `@noble/ciphers/aes.js#rngAesCtrDrbg256` (byte-identical to Python's AES256_CTR_DRBG per empirical test). No fixture schema additions — `drbgSeed` alone is the trust anchor. Story 2-1 consumes via `rngAesCtrDrbg256(hexToBytes(v.drbgSeed)).randomBytes(48)` for keygen inner_seed; Story 2-3 advances the same DRBG past the 48 B keygen draw to get the 88 B signing randomness. See `docs/amendments.md` §A-005.
...
- **G3 (Falcon-ETH keygen byte-identity):** `keygenInternal(drbgSeed)` → assert `(pk, sk)` vs `.rsp` per vector. Uses G1-verified XOF. ≥100 vectors from `ethfalcon512-KAT.rsp`. [**A-005**: reinterpreted surface — `keygenInternal` takes `innerSeed: Uint8Array` (the 48 B post-DRBG draw), NOT `drbgSeed`. Inner seed is derived TS-side at test time via `rngAesCtrDrbg256(hexToBytes(v.drbgSeed)).randomBytes(48)` — no fixture field added. Keygen XOF is SHAKE256 (not Keccak-PRG); noble's `falcon512.keygen(innerSeed)` is byte-identical to ETHFALCON. G1-verified Keccak-PRG is load-bearing for G4 only. See `docs/amendments.md` §A-005.]
- **G4 (Falcon-ETH signer byte-identity):** `signWithDrbgRnd(sk, msg, drbgSeed)` (KAT surface) → assert 1064-byte `salt || s2` vs `.rsp` `sm[:-mlen]` per vector. Uses G1-verified XOF + G2-verified HashToPoint + G3-verified sk. ≥100 vectors. [**A-005**: reinterpreted surface — KAT signer is `signWithKatBytes(sk, msg, reader: BytesReader)` (or equivalent; Story 2-3 picks final name). Reader is sourced TS-side: `const drbg = rngAesCtrDrbg256(hexToBytes(v.drbgSeed)); drbg.randomBytes(48); const reader = { read: (n) => drbg.randomBytes(n) }` — advances the DRBG past the keygen draw, then consumes the 40 + 48 = 88 B signer randomness. No fixture field. See `docs/amendments.md` §A-005 "Forward contract for Story 2-3".] [**A-006 SUPERSEDES**: A-005's 285-LOC `signRaw` + `FFSampler` + `HashToPoint` source-transplant is DROPPED in favour of Strategy E — fork-side HashToPoint injection at `github.com/LimeChain/noble-post-quantum-eth#falcon-eth-hashtopoint-injection`. Our repo instantiates `falcon512paddedEth = genFalcon({ ...falcon512paddedOpts, hashToPoint: hashToPointEVM })` and wraps `falcon512paddedEth.sign(...) + encodeSignatureForZKNOX` (Story 2-2 helper handles the 666 B → 1064 B encoding). `signWithXofInstrumented` + DD-10 XofFactory parameterization DROPPED. See `docs/amendments.md` §A-006.]
- **G5 (pk-transform byte-identity):** Primary — `preparePublicKeyForDeployment(rawPk, keccakXofFactory)` returns `Hex`; assert byte-equality vs fixture `reshapedPublicKey` (also `Hex`). Structural sub-check — `pkToNttCompact(rawPk, keccakXofFactory)` returns `bigint[]` of length 32 for every vector (debug anchor if `Hex` comparison fails). Uses G1-verified XOF + G3-verified pk. ≥100 vectors. **Pk-format divergence probe (lesson 5.1 carry-over):** spot-check vec 0's `reshapedPublicKey.length` against TS output length BEFORE writing the G5 test; if formats diverge, pick structural-decode oracle up front (`test/signers/mldsa-encoding.pk-transform.kat.test.ts` ~200 LOC template). [**A-007**: divergence confirmed at Story 2-4 T1 RED — fixture is `uint256[32]` (1024 B), `encodePublicKeyForZKNOX` helper (and `preparePublicKeyForDeployment` delegate) emits `uint256[]` (1088 B, required by on-chain `setKey`). Structural-decode oracle fallback taken; AC-1 amended to element-wise `bigint[]` equality after decoding both sides. See `docs/amendments.md` §A-007.]
...
2. **Story 2 — G1 Keccak-PRG byte-identity vs ETHFALCON.** Capture G1 Layer-2 vectors from ETHFALCON's `Keccak256PRNG(a, b)` [**A-004**: actual ETHFALCON class is `KeccakPRNG()` (0-arg); `Keccak256PRNG(a, b)` is the ETHDILITHIUM wrapper — see `docs/amendments.md` A-004]. Compare to existing `keccakXofFactory` output. If identical, ship G1 test as thin assertion; if divergent, port `falconKeccakXofFactory` adapter. **May collapse into Story 3** if G1 is trivially identical (likely — ETHDILITHIUM copied from ETHFALCON verbatim, only wrapper API differs).
3. **Story 3 — Falcon-ETH core + keygen port → G3 PASS.** Fork noble math into `falcon-eth.core.ts`; implement sampler with `XofFactory` parameterization (DD-10); expose `keygenInternal(drbgSeed)` that replays `AES256_CTR_DRBG` to derive internal entropy. **HashToPoint is NOT a keygen dependency** — it's deferred to Story 4. G3 KAT over 100 `.rsp` vectors. Module header `@delta-from-falcon` JSDoc lands with `FALCON_DELTA_HEADINGS` test (AC-D-3). [**A-006 SUPERSEDES**: Strategy E fork-injection replaces the 285-LOC source-transplant. `falcon-eth.core.ts` holds `hashToPointEVM` (Story 2-2) + `falcon512paddedEth` instantiation (Story 2-3 T2); no forked math, no `XofFactory` parameterization. See `docs/amendments.md` §A-006.]
4. **Story 4 — HashToPoint port + Signer port → G2 + G4 PASS.** **Task 1 (HashToPoint isolated):** port `hashToPointEVM(salt, msg)` as standalone shared-core export; run G2 against 6 transcribed vectors; first-differing-coefficient ±4 context on mismatch. **Tasks 2–5 (Signer port):** implement hybrid fork per DD-15 (reuse noble's `ffSampling`/`splitFFT`/`mergeFFT`, fork sampler to Keccak-PRG), consume Story 4 Task 1's HashToPoint in the sign loop, expose `signWithDrbgRnd(sk, msg, drbgSeed)` for KAT surface + `signWithXofInstrumented` sibling (DD-10). Production `signUserOp` on top. G4 byte-identity over 100 `.rsp` vectors. Hedged-sign, deterministic-sign, SignerInputError validation. **Story 4 ships two gates (G2 + G4) because HashToPoint is cheap isolated + load-bearing for signer debugging.** [**A-006 SUPERSEDES**: `signWithDrbgRnd` / `signWithXofInstrumented` / hybrid-fork sampler DROPPED; Strategy E uses fork-side HashToPoint injection + `signWithKatBytes(sk, msg, reader)` KAT surface. G4 passed 100/100 via `signWithKatBytes` at Story 2-3 Task 4 commit `20b740a`. See `docs/amendments.md` §A-006.]
...
| `INVALID_DRBG_SEED_LENGTH` | `SignerInputError` | `drbgSeed.length !== 48` (KAT surface; matches NIST AES256_CTR_DRBG seed size) [**A-006 SUPERSEDES**: this error code is DROPPED from the Story 2-3 surface — no signer function takes `drbgSeed` directly post-Strategy-E. `rngAesCtrDrbg256` is imported from `@noble/ciphers/aes.js` at test time only; its input validation is noble's responsibility. Story 2-3 introduces `SIGNING_BYTES_EXHAUSTED` for reader over-draw (both KAT and production signer surfaces). See `docs/amendments.md` §A-006 + `test/signers/errors.ts`.] |
...
> [**A-006 SUPERSEDES**: the three interface sketches below (`falcon-eth.ts`, `falcon-eth.kat-internal.ts`, `falcon-eth.core.ts`) reflect the pre-Strategy-E plan — `signWithXof(…, XofFactory)`, `signWithXofInstrumented`, `signWithDrbgRnd(sk, msg, drbgSeed)`, `@delta-from-falcon` headers, `XofFactory`-parameterized core — ALL DROPPED. Actual Story 2-3 surfaces (per `docs/stories/2-3.md` + `docs/amendments.md` §A-006):
>
> - `test/signers/falcon-eth.ts` — `keygen(): Keypair` + `signUserOp(sk, userOp, entryPoint, chainId): Promise<PackedUserOperation>` (88 B `crypto.getRandomValues` → inline `BytesReader` → `falcon512paddedEth.sign` → `encodeSignatureForZKNOX` → 1064 B hex)
> - `test/signers/falcon-eth.kat-internal.ts` — `keygenInternal(innerSeed): Keypair` (Story 2-1) + `BytesReader` interface + `signWithKatBytes(sk, msg, reader: BytesReader): Uint8Array` (Story 2-3 T3). Input validation: `INVALID_SECRET_KEY_LENGTH`, `INVALID_MESSAGE`, `SIGNING_BYTES_EXHAUSTED`. NO `INVALID_DRBG_SEED_LENGTH` or `INVALID_CTX_LENGTH`.
> - `test/signers/falcon-eth.core.ts` — `hashToPointEVM(salt, msg): Uint16Array` (Story 2-2) + `falcon512paddedEth: Falcon = genFalcon({ ...falcon512paddedOpts, hashToPoint: hashToPointEVM })` (Story 2-3 T2). NO `signWithXof` / `signWithXofInstrumented` / `pkToNttCompact` / `preparePublicKeyForDeployment` in 2-3's scope (pk-transform is Story 2-4 via the existing `falcon-encoding.ts` helpers).
>
> The sketches below are preserved as historical architecture-intent. See `docs/amendments.md` §A-006 for Strategy E's actual shape.]
...
| G4 — Signer KAT | `test/signers/falcon-eth.sign.kat.test.ts` (new) | `signWithDrbgRnd(sk, msg, drbgSeed)` byte-identity over 100 `.rsp` vectors. Instrumented pass asserts rejection-counter positive (HashToPoint rejection loop). [**A-006 SUPERSEDES**: G4 oracle is `signWithKatBytes(sk, msg, reader)` — `reader` is a `BytesReader` derived TS-side from `rngAesCtrDrbg256(hexToBytes(v.drbgSeed))` advanced past the 48 B keygen draw (A-005 DRBG contract). Instrumented pass / `signWithXofInstrumented` DROPPED (G4 byte-identity subsumes rejection-loop predicate). See `docs/amendments.md` §A-006.] | AC-FR-13, AC-FR-6, AC-NFR-3 |
| G4 — Signer non-KAT | `test/signers/falcon-eth.sign.test.ts` (new) | Input validation (SignerInputError per field), production hedged sign (2 calls → different salt prefixes), deterministic sign [**A-006 SUPERSEDES**: AC-2 roundtrip-verify clause deferred to Story 2-4 G6 (see `docs/stories/2-3.md` §"Acceptance Criteria" AC-2 and F2 code-review finding); this story validates salt-divergence + structural-length invariants. See `docs/amendments.md` §A-006.] | AC-FR-4, AC-FR-5, AC-FR-18 |
...
| DD-10 | Parameterize-by-factory: `XofFactory` passed to every call-site; sibling-export `signWithXofInstrumented` for rejection-counter; zero module-level XOF state | **LOCKED** (Research) | AC-A-5 grep enforces extension to `test/signers/{mldsa,falcon}-eth*.ts` glob [**A-006 SUPERSEDES** for Story 2-3 signer path: DD-10's `XofFactory` parameterization + `signWithXofInstrumented` sibling is REPLACED by Strategy E (fork-side `hashToPoint?` factory-option injection; single `falcon512paddedEth` instance — no runtime XOF swap, no rejection-counter instrumentation). DD-10 remains valid for ml-dsa-eth's path. See `docs/amendments.md` §A-006.] |
| DD-11 | Oracle chain PRG → keygen → signer → pk-transform → verifier, each gate byte-identity | **LOCKED** (Research) | — |
| DD-13 | G1 Keccak-PRG byte-identity verification REQUIRED (not skippable) against ETHFALCON's `Keccak256PRNG(a, b)` wrapper [**A-004**: actual ETHFALCON class is `KeccakPRNG()` (0-arg); `Keccak256PRNG(a, b)` is the ETHDILITHIUM wrapper — see `docs/amendments.md` A-004. The "wrapper APIs differ" rationale holds, only the two names are inverted in this row's prose.] | **LOCKED** (Research) | ETHFALCON is upstream origin of keccak_prng.py; ETHDILITHIUM wrapper API diverges (`KeccakPRNG()` vs `Keccak256PRNG(a, b)`) [**A-004**: names inverted — see above]; internal mechanics may be identical but verification is cheap-to-land and prevents silent downstream drift |
