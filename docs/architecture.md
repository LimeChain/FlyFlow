...
```jsonc
{
  ...
  "source": {
    ...
    "drbgDerivation": "AES256_CTR_DRBG(drbgSeed).random_bytes(N) — N TBD by Python-ref audit (PRE_G4_DRBG_PROBE)",  // [A-005]: DRBG runs at Python fixture-gen time only; TS never replays. Story 2-1 T1 adds `innerSeed` (48 B) + `signingDrbg` (variable B) fixture fields to the schema below. N per vector is captured via RecordingDRBG wrapper in Python. See `docs/amendments.md` §A-005.
...
- **G3 (Falcon-ETH keygen byte-identity):** `keygenInternal(drbgSeed)` → assert `(pk, sk)` vs `.rsp` per vector. Uses G1-verified XOF. ≥100 vectors from `ethfalcon512-KAT.rsp`. [**A-005**: superseded surface — `keygenInternal` takes `innerSeed: Uint8Array` (the precomputed 48 B post-DRBG draw from fixture), NOT `drbgSeed`. Keygen XOF is SHAKE256 (not Keccak-PRG); noble's `falcon512.keygen(innerSeed)` is byte-identical to ETHFALCON. G1-verified Keccak-PRG is load-bearing for G4 only. See `docs/amendments.md` §A-005.]
- **G4 (Falcon-ETH signer byte-identity):** `signWithDrbgRnd(sk, msg, drbgSeed)` (KAT surface) → assert 1064-byte `salt || s2` vs `.rsp` `sm[:-mlen]` per vector. Uses G1-verified XOF + G2-verified HashToPoint + G3-verified sk. ≥100 vectors. [**A-005**: superseded surface — KAT signer is `signWithKatBytes(sk, msg, reader: BytesReader)` (or equivalent; Story 2-3 picks final name). Reader wraps fixture's `signingDrbg` field (precomputed post-keygen DRBG draws). No TS-side DRBG replay. See `docs/amendments.md` §A-005 "Forward contract for Story 2-3".]
